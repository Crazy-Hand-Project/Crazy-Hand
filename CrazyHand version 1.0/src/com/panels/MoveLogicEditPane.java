...
public class MoveLogicEditPane extends JPanel implements ActionListener {
	...
	int[][] charNames = {								//.    d    a    t
		...
		private LogicNode[] getLogicTableForCharacter(int s) {
			FileIO.init();
			String[] subs = SubactionPanel.getDefaultSubactions();
			...
			while(!finishedLooking){
				...
				*/
				...
				if(MeleeEdit.selected!=Character.YOSHI_ID){
					...
					if(compareString("Pl" + Character.characters[MeleeEdit.selected].id + ".dat")){
						finishedLooking=true;
						break;
					}
				}
				else{
					if(compareString("yoshi matanim")){
						finishedLooking=true;
						break;
					}
				}
		...
		public boolean compareString(String s){
			for(int i = 0; i < s.length(); i++){
				if(FileIO.readByte()!=(byte)s.charAt(i)){
					return false;
				}
			}
			return true;
